...
export default function WhatsAppPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-8" style={{ background: '#121317' }}>
        {!instance ? (
          // Not connected - Show connect button
          <div className="text-center space-y-6">
            <div className="w-24 h-24 mx-auto rounded-full flex items-center justify-center" style={{ background: 'rgba(199,218,248,0.1)' }}>
              <svg className="w-12 h-12" style={{ color: '#C7DAF8' }} fill="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              ...
              <button
                ...
                style={{
                  background: connecting ? 'rgba(199,218,248,0.3)' : 'linear-gradient(135deg, #C7DAF8, #C7DAF8)',
                  color: '#121317',
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: instance.profile_picture_url ? 'transparent' : 'rgba(199,218,248,0.1)' }}>
                  {instance.profile_picture_url ? (
                    ...
                    <svg className="w-8 h-8" style={{ color: '#C7DAF8' }} fill="currentColor" viewBox="0 0 24 24">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                    </svg>
                  )}
                </div>
                <div>
                  ...
                  {instance.phone_number && (
                    <p style={{ color: 'rgba(255,255,255,0.6)' }}>{instance.phone_number}</p>
                  )}
                </div>
              </div>
            ...
            {(qrCode || instance.status === 'connecting') && (
              <div className="rounded-2xl p-8 text-center" style={{ background: 'rgba(199,218,248,0.05)' }}>
                ...
                {qrCode ? (
                  ...
                  <div className="w-64 h-64 mx-auto flex items-center justify-center rounded-xl" style={{ background: 'rgba(255,255,255,0.1)' }}>
                    <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-[#C7DAF8]"></div>
                  </div>
                )}
                {pairingCode && (
                  <div className="mt-4">
                    ...
                    <code className="px-4 py-2 rounded-lg text-xl font-bold" style={{ background: 'rgba(0,0,0,0.3)', color: '#C7DAF8' }}>
                      {pairingCode}
                    </code>
                  </div>
                )}
            ...
            {instance.status === 'connected' && (
              <div className="rounded-2xl p-6" style={{ background: 'rgba(199,218,248,0.1)' }}>
                <div className="flex items-center gap-3 mb-4">
                  <svg className="w-6 h-6" style={{ color: '#C7DAF8' }} fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                  </svg>
                  <h3 className="font-bold" style={{ color: '#C7DAF8' }}>
            ...
            <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.05)' }}>
              <h4 className="text-sm font-bold mb-2" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {isRTL ? 'تفاصيل المثيل:' : 'Instance Details:'}
              </h4>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span style={{ color: 'rgba(255,255,255,0.4)' }}>{isRTL ? 'اسم المثيل:' : 'Instance Name:'}</span>
                  <div style={{ color: '#FFFFFF' }}>{instance.instance_name}</div>
                </div>
                <div>
                  <span style={{ color: 'rgba(255,255,255,0.4)' }}>{isRTL ? 'الحالة:' : 'Status:'}</span>
