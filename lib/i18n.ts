export const translations = {
  ar: {
    // Navigation
    nav: {
      dashboard: 'الرئيسية',
      inbox: 'الرسائل',
      channels: 'القنوات',
      tools: 'الأدوات',
      blog: 'المدونة',
      reports: 'التقارير',
      reputation: 'السمعة',
      aiKnowledge: 'معرفة الذكاء الاصطناعي',
      billing: 'الفوترة',
      settings: 'الإعدادات',
      login: 'تسجيل الدخول',
      startFree: 'ابدأ مجاناً',
      notifications: 'الإشعارات',
      whatsapp: 'واتساب',
      content: 'المحتوى',
    },
    hero: {
      badge: 'ذكاء اصطناعي · AI Automation',
      headline1: 'موظفك الذكي',
      headline2: 'الذي لا ينام',
      subheadline: 'منصة ذكاء اصطناعي ترد‌ على رسائل عملائك تلقائياً، تنشئ المحتوى، وتحمي سمعتك على Google — على مدار الساعة.',
      ctaPrimary: 'ابدأ مجاناً — 14 يوم مجاني',
      ctaSecondary: 'شاهد كيف يعمل',
      socialProof: 'يثق به أكثر من 500+ صاحب عمل',
      stats: {
        messages: '340 رسالة تم الرد عليها',
        replyTime: 'وقت الرد: 8 ثوانِ',
        rating: '4.9 تقييم Google',
      },
    },
    trustedBy: {
      title: 'يستخدمه أصحاب المطاعم، العيادات، الكافيهات، المتاجر...',
    },
    problems: {
      title: 'هل تعاني من هذا كل يوم؟',
      items: [
        { icon: '🌙', title: 'رسائل بلا رد في المساء', desc: 'عملاؤك يرسلون بعد الدوام ولا أحد يرد' },
        { icon: '🔄', title: 'أسئلة متكررة تأكل وقتك', desc: 'نفس الأسئلة كل يوم تستنزف طاقتك وفريقك' },
        { icon: '⭐', title: 'تقييمات سلبية بلا رد', desc: 'تقييم سلبي واحد بلا رد يضر بعملك' },
        { icon: '📱', title: 'قنوات مشتتة في كل مكان', desc: 'واتساب، انستغرام، فيسبوك — لا تقدر تتابع الكل' },
      ],
    },
    features: {
      title: 'حل واحد لكل المشاكل',
      items: [
        { icon: '📥', title: 'صندوق الوارد الموحد', desc: 'كل رسائلك من جميع القنوات في مكان واحد. تابع المحادثات بسهولة دون تبديل التطبيقات.' },
        { icon: '🎭', title: 'الرد بأسلوبك الشخصي', desc: 'الذكاء الاصطناعي يتعلم أسلوبك ويرد كما لو كنت أنت بنفسك.' },
        { icon: '🛡️', title: 'Before They Complain', desc: 'نظام يكتشف العملاء غير الراضين ويتواصل معهم قبل أن يتركوا تقييماً سلبياً.' },
        { icon: '✍️', title: 'محرك المحتوى التلقائي', desc: 'ينشئ محتوى يومياً لسوشال ميديا مناسباً لعملك بدون جهد منك.' },
        { icon: '🔍', title: 'صائد الكلمات المفتاحية', desc: 'يرصد المحادثات التي تحتاج تدخلاً بشرياً ويُنبهك فوراً.' },
        { icon: '📊', title: 'التقرير الأسبوعي الذكي', desc: 'تقرير شامل كل أسبوع عن أداء ردودك ورضا العملاء وفرص التحسين.' },
      ],
    },
    howItWorks: {
      title: '3 خطوات وأنت جاهز',
      steps: [
        { num: '01', title: 'سجّل وارتبط', desc: 'اربط قنواتك في أقل من 3 دقائق — انستغرام، واتساب، فيسبوك، Gmail وأكثر.' },
        { num: '02', title: 'عرّف عملك', desc: 'أخبر الذكاء الاصطناعي عن طبيعة عملك، أسلوبك، وأسئلتك الشائعة.' },
        { num: '03', title: 'نَم بهدوء', desc: 'الذكاء الاصطناعي يرد على عملائك، ينشر محتواك، ويحمي سمعتك — تلقائياً.' },
      ],
    },
    pricing: {
      title: 'اختر الباقة المناسبة',
      monthly: 'شهري',
      annual: 'سنوي',
      annualSave: 'وفر 20%',
      mostPopular: 'الأكثر طلباً',
      startFree: 'ابدأ الآن',
      plans: [
        { name: 'Starter', nameAr: 'الشخصية', price: 29, desc: 'مثالي للأعمال الصغيرة', features: ['قناة واحدة', '500 رد/شهر', 'ردود AI أساسية', 'تقرير شهري', 'دعم بالبريد'] },
        { name: 'Business', nameAr: 'الأعمال', price: 59, popular: true, desc: 'الأنسب لمعظم الأعمال', features: ['5 قنوات', '2,000 رد/شهر', 'Before They Complain', 'محتوى تلقائي يومي', 'تقرير أسبوعي', 'دعم مباشر'] },
        { name: 'Growth', nameAr: 'النمو الشامل', price: 89, desc: 'للأعمال في مرحلة التوسع', features: ['10 قنوات', '5,000 رد/شهر', 'كل مميزات Business', 'صائد الكلمات المفتاحية', 'تحليلات متقدمة', 'API وصول'] },
        { name: 'Agency', nameAr: 'الوكالات', price: 249, desc: 'لإدارة عملاء متعددين', features: ['قنوات غير محدودة', 'ردود غير محدودة', 'لوحة تحكم وكالة', 'White-label', 'مدير حساب مخصص', 'SLA مضمون'] },
      ],
    },
    testimonials: {
      title: 'ماذا يقول عملاؤنا',
      items: [
        { name: 'أحمد السالم', business: 'مطعم، الرياض', rating: 5, quote: 'بعد ناز أصبحت ردودنا فورية حتى وقت النوم. العملاء لاحظوا الفرق وارتفعت التقييمات بشكل ملحوظ.' },
        { name: 'سارة المطيري', business: 'عيادة تجميل، جدة', rating: 5, quote: 'كنت أضيع ساعات يومياً في الرد على نفس الأسئلة. الآن الذكاء الاصطناعي يتكفل بكل شيء وأنا أركز على عملي.' },
        { name: 'محمد الزهراني', business: 'كافيه، الدمام', rating: 5, quote: 'نظام Before They Complain أنقذني أكثر من مرة. أصبحت أعرف بمشكلة العميل قبل أن يتركني تقييماً سيئاً.' },
      ],
    },
    faq: {
      title: 'الأسئلة الشائعة',
      items: [
        { q: 'هل أحتاج خبرة تقنية لاستخدام ناز؟', a: 'لا إطلاقاً. ناز مصمم ليكون سهل الاستخدام لأي صاحب عمل. الإعداد يستغرق أقل من 10 دقائق.' },
        { q: 'هل الذكاء الاصطناعي سيرد بدلاً مني أم يقترح ردوداً؟', a: 'يمكنك اختيار الوضعين. إما رد تلقائي كامل أو اقتراح ردود تراجعها وتوافق عليها.' },
        { q: 'ما هي القنوات المدعومة؟', a: 'انستغرام، فيسبوك، واتساب، Gmail، تيك توك، وX (تويتر). قريباً: سناب شات وتيليغرام.' },
        { q: 'هل بياناتي آمنة؟', a: 'نعم. نستخدم تشفير SSL من طرف لطرف وبياناتك لا تُشارك مع أي طرف ثالث.' },
        { q: 'هل يمكنني إلغاء الاشتراك في أي وقت؟', a: 'نعم، يمكنك إلغاء اشتراكك في أي وقت دون أي رسوم إضافية.' },
        { q: 'هل هناك فترة تجربة مجانية؟', a: 'نعم! كل الباقات تأتي مع 14 يوم تجربة مجانية كاملة بدون بطاقة ائتمان.' },
      ],
    },
    finalCta: {
      title: 'ابدأ اليوم — الرد الأول مجاناً',
      btn: 'ابدأ تجربتك المجانية',
      noCc: 'لا يتطلب بطاقة ائتمانية',
    },
    // Auth
    auth: {
      login: 'تسجيل الدخول',
      register: 'إنشاء حساب',
      email: 'البريد الإلكتروني',
      password: 'كلمة المرور',
      confirmPassword: 'تأكيد كلمة المرور',
      name: 'الاسم',
      forgotPassword: 'نسيت كلمة المرور؟',
      noAccount: 'ليس لديك حساب؟',
      hasAccount: 'لديك حساب بالفعل؟',
      signUp: 'إنشاء حساب',
      signIn: 'تسجيل الدخول',
      loginSuccess: 'تم تسجيل الدخول بنجاح',
      registerSuccess: 'تم إنشاء الحساب بنجاح',
      loginError: 'فشل تسجيل الدخول',
      registerError: 'فشل إنشاء الحساب',
      invalidEmail: 'البريد الإلكتروني غير صالح',
      invalidPassword: 'كلمة المرور يجب أن تكون 8 أحرف على الأقل',
      passwordsMismatch: 'كلمات المرور غير متطابقة',
      continueWithGoogle: 'المتابعة عبر Google',
      continueWithFacebook: 'المتابعة عبر Facebook',
      orDivider: 'أو',
      authFailed: 'فشل تسجيل الدخول. حاول مرة أخرى',
      // Auth Panel
      welcomeBack: 'مرحباً بعودتك',
      yourSystemAwaits: 'نظامك في انتظارك',
      loginToAccess: 'سجل دخولك للوصول إلى لوحة تحكم الذكاء الاصطناعي',
      startFree: 'ابدأ مجاناً',
      noCreditCard: 'بدون بطاقة ائتمانية',
      joinToday: 'انضم اليوم',
      systemOnline: 'SYSTEM ONLINE',
      stats: {
        businesses: 'عمل تجاري',
        autoReplies: 'رد تلقائي',
        avgReply: 'متوسط الرد',
      },
      features: {
        reply8sec: 'رد في 8 ثوانٍ على كل رسالة',
        aiUnderstands: 'ذكاء اصطناعي يفهم لهجتك',
        instantReports: 'تقارير لحظية لأداء نشاطك',
        freeTrial: '14 يوم تجربة مجانية كاملة',
        noCreditCard: 'لا حاجة لبطاقة ائتمانية',
        ready5min: 'جاهز في أقل من 5 دقائق',
      },
      liveActivity: 'LIVE ACTIVITY',
      activity: {
        ahmedWhatsapp: 'أحمد — WhatsApp',
        replied: 'تم الرد ✓',
        saraInstagram: 'سارة — Instagram',
        potentialLead: 'عميل محتمل ⚡',
      },
      allRightsReserved: '© 2025 Naz. All rights reserved.',
      byContinuing: 'بالمتابعة توافق على',
      termsPrivacy: 'الشروط والسياسة',
      // Password strength
      passwordWeak: 'ضعيفة',
      passwordFair: 'متوسطة',
      passwordStrong: 'قوية',
      // Register page
      startJourney: 'ابدأ رحلتك',
      createAccountActivate: 'أنشئ حسابك وفعّل موظفك الذكي في دقائق',
      createAccountActivateEn: 'Create your account and activate your AI in minutes',
      // Common
      signingIn: 'جاري الدخول...',
      signingInEn: 'Signing in...',
      creatingAccount: 'جاري إنشاء الحساب...',
      creatingAccountEn: 'Creating account...',
      // Trust badges
      secure: 'آمن',
      secureEn: 'Secure',
      daysFree: '14 يوم مجاني',
      daysFreeEn: '14 days free',
      instantSetup: 'جاهز فوراً',
      instantSetupEn: 'Instant setup',
    },
    // AI Knowledge
    aiKnowledge: {
      title: 'معرفة الذكاء الاصطناعي والتعليمات',
      subtitle: 'خصص كيف يفهم الذكاء الاصطناعي نشاطك التجاري ويرد على العملاء',
      knowledgeBase: 'قاعدة المعرفة',
      knowledgeBaseDesc: 'ارفع ملفات PDF أو Excel تحتوي على معلومات نشاطك التجاري، القوائم، قوائم الأسعار، الأسئلة الشائعة، أو أي مستندات مرجعية.',
      uploadFile: 'رفع ملف',
      uploadFileDesc: 'رفع ملفات PDF أو Excel (حد أقصى 10 ميجابايت)',
      noFiles: 'لم يتم رفع أي ملفات بعد',
      uploadedAt: 'تم الرفع',
      deleteFile: 'حذف',
      deleteConfirm: 'هل أنت متأكد من حذف هذا الملف؟',
      extractError: 'فشل استخراج النص من الملف',
      uploadSuccess: 'تم رفع الملف بنجاح',
      uploadError: 'فشل رفع الملف',
      fileDeleted: 'تم حذف الملف بنجاح',
      customInstructions: 'تعليمات الذكاء الاصطناعي المخصصة',
      customInstructionsDesc: 'أخبر الذكاء الاصطناعي كيف يتصرف، ما النبرة التي يستخدمها، والقواعد المحددة لنشاطك التجاري.',
      instructionsPlaceholder: 'رحب بالعملاء دائماً بشكل دافئ. لا تعد بالخصومات دون التحقق من المالك. إذا سأل شخص عن التوصيل خارج الرياض، أخبره بالاتصال بنا مباشرة.',
      saveInstructions: 'حفظ التعليمات',
      instructionsSaved: 'تم حفظ التعليمات بنجاح',
      instructionsError: 'فشل حفظ التعليمات',
      testAi: 'اختبار رد الذكاء الاصطناعي',
      testAiDesc: 'شاهد كيف يرد الذكاء الاصطناعي باستخدام معرفتك الحالية وتعليماتك.',
      testQuestion: 'سؤال اختبار',
      testQuestionPlaceholder: 'اكتب سؤال عميل لاختبار رد الذكاء الاصطناعي...',
      testResponse: 'رد الذكاء الاصطناعي',
      testButton: 'اختبار الرد',
      testing: 'جاري الاختبار...',
      testError: 'فشل إنشاء رد الاختبار',
      supportedFormats: 'التنسيقات المدعومة: PDF، Excel (XLSX، XLS)',
      maxSize: 'الحد الأقصى لحجم الملف: 10 ميجابايت',
    },
    // Dashboard
    dashboard: {
      title: 'لوحة التحكم',
      totalMessages: 'إجمالي الرسائل',
      aiReplies: 'ردود الذكاء الاصطناعي',
      responseRate: 'معدل الاستجابة',
      activeChannels: 'القنوات النشطة',
      recentConversations: 'المحادثات الأخيرة',
      noChannels: 'لم يتم ربط أي قنوات بعد',
      connectChannel: 'ربط قناة',
      vsYesterday: 'مقارنة بالأمس',
    },
    // Inbox
    inbox: {
      title: 'الرسائل',
      noConversations: 'لا توجد محادثات',
      noResults: 'لا توجد نتائج',
      noMessages: 'لا توجد رسائل',
      selectConversation: 'اختر محادثة للبدء',
      typeMessage: 'اكتب رسالتك...',
      send: 'إرسال',
      aiMode: 'الرد الآلي',
      aiModeActive: 'الرد الآلي مفعل',
      aiModeInactive: 'الرد الآلي معطل',
      typing: 'جاري الكتابة...',
      manualReply: 'رد يدوي',
      aiReply: 'رد ذكاء اصطناعي',
    },
    // Channels
    channels: {
      title: 'القنوات',
      subtitle: 'اربط قنواتك وفعّل الرد الآلي بالذكاء الاصطناعي',
      connect: 'ربط',
      connected: 'متصل',
      notConnected: 'غير مرتبط',
      disconnected: 'غير متصل',
      reconnect: 'إعادة الاتصال',
      disconnect: 'فصل',
      autoReplyActive: 'الرد الآلي مفعل',
      autoReplyPaused: 'الرد الآلي معطل',
      connectFacebook: 'ربط فيسبوك',
      connectInstagram: 'ربط إنستغرام',
      connectGmail: 'ربط جيميل',
      connectWhatsApp: 'ربط واتساب',
      lastMessage: 'آخر رسالة',
      noChannels: 'لم يتم ربط أي قنوات',
      willNeedSignIn: 'ستحتاج إلى تسجيل الدخول للسماح بالوصول',
      permissionText: 'سيطلب Naz إذناً للوصول إلى رسائل {channel} للرد عليها تلقائياً بالذكاء الاصطناعي.',
      confirmDisconnect: 'هل أنت متأكد من فصل هذه القناة؟',
      connectedSuccess: 'تم ربط القناة بنجاح',
      connectionCancelled: 'تم إلغاء الربط',
      connectionFailed: 'فشل ربط القناة',
      disconnected2: 'تم فصل القناة',
      disconnectFailed: 'فشل فصل القناة',
      updateFailed: 'فشل تحديث الإعدادات',
      aiOn: 'الذكاء الاصطناعي: مفعّل',
      aiOff: 'الذكاء الاصطناعي: معطل',
      aiEnabled: 'تم تفعيل الذكاء الاصطناعي',
      aiDisabled: 'تم تعطيل الذكاء الاصطناعي',
      businessPlan: 'خطة الأعمال',
      upgradeNow: 'ترقية الآن',
    },
    // Tools
    tools: {
      title: 'الأدوات المجانية',
      tryIt: 'جربها',
      freeUses: 'استخدامات مجانية متبقية اليوم',
      limitReached: 'لقد وصلت إلى الحد اليومي',
      signUpForUnlimited: 'سجل للوصول غير المحدود',
    },
    // Blog
    blog: {
      title: 'المدونة',
      readMore: 'اقرأ المزيد',
      backToBlog: 'العودة للمدونة',
      relatedTools: 'أدوات مجانية ذات صلة',
      upsell: 'اكتشف كيف يمكن لـ Naz Autoreply أن يضاعف مبيعاتك',
      tryFree: 'جرّب مجاناً',
    },
    // Common
    common: {
      loading: 'جاري التحميل...',
      error: 'حدث خطأ',
      retry: 'إعادة المحاولة',
      save: 'حفظ',
      cancel: 'إلغاء',
      delete: 'حذف',
      edit: 'تعديل',
      close: 'إغلاق',
      search: 'بحث',
      filter: 'تصفية',
      all: 'الكل',
      profile: 'الملف الشخصي',
      logout: 'تسجيل الخروج',
      continue: 'متابعة',
      back: 'رجوع',
    },
    // Footer
    footer: {
      tagline: 'موظفك الذكي الذي لا ينام',
      product: 'المنتج',
      productLinks: ['المميزات', 'التسعير', 'التكامل', 'الأمان'],
      company: 'الشركة',
      companyLinks: ['عن ناز', 'المدونة', 'الوظائف', 'اتصل بنا'],
      support: 'الدعم',
      supportLinks: ['مركز المساعدة', 'الوثائق', 'حالة النظام', 'الخصوصية'],
      copyright: '© 2026 ناز. جميع الحقوق محفوظة',
    },
    // Empty States
    empty: {
      noData: 'لا توجد بيانات',
      noMessages: 'لا توجد رسائل',
      noChannels: 'لم يتم ربط أي قنوات',
      noReports: 'لا توجد تقارير',
      noPosts: 'لا توجد مقالات',
      getStarted: 'ابدأ الآن',
    },
  },
  en: {
    // Navigation
    nav: {
      dashboard: 'Dashboard',
      inbox: 'Inbox',
      channels: 'Channels',
      tools: 'Tools',
      blog: 'Blog',
      reports: 'Reports',
      reputation: 'Reputation',
      aiKnowledge: 'AI Knowledge',
      billing: 'Billing',
      settings: 'Settings',
      login: 'Login',
      startFree: 'Start Free',
      notifications: 'Notifications',
      whatsapp: 'WhatsApp',
      content: 'Content',
    },
    hero: {
      badge: 'AI Automation · ذكاء اصطناعي',
      headline1: 'The AI Employee',
      headline2: 'That Never Sleeps',
      subheadline: 'An AI platform that automatically replies to customer messages, creates content, and protects your Google reputation — 24/7.',
      ctaPrimary: 'Start Free — 14-Day Trial',
      ctaSecondary: 'Watch How It Works',
      socialProof: 'Trusted by 500+ business owners',
      stats: {
        messages: '340 messages replied',
        replyTime: 'Reply time: 8 seconds',
        rating: '4.9 Google Rating',
      },
    },
    trustedBy: {
      title: 'Trusted by restaurants, clinics, cafés, and stores...',
    },
    problems: {
      title: 'Do You Face These Daily?',
      items: [
        { icon: '🌙', title: 'Unanswered Messages at Night', desc: 'Customers message after hours and no one replies' },
        { icon: '🔄', title: 'Repetitive Questions Drain You', desc: 'Same questions every day exhaust you and your team' },
        { icon: '⭐', title: 'Negative Reviews Without Response', desc: 'One unanswered negative review hurts your business' },
        { icon: '📱', title: 'Channels Scattered Everywhere', desc: 'WhatsApp, Instagram, Facebook — impossible to track all' },
      ],
    },
    features: {
      title: 'One Solution for Every Problem',
      items: [
        { icon: '📥', title: 'Unified Inbox', desc: 'All messages from every channel in one place. Track conversations without switching apps.' },
        { icon: '🎭', title: 'Reply in Your Voice', desc: 'AI learns your style and replies exactly as you would.' },
        { icon: '🛡️', title: 'Before They Complain', desc: 'Detects unhappy customers and reaches out before they leave a bad review.' },
        { icon: '✍️', title: 'Auto Content Engine', desc: 'Generates daily social media content tailored to your business — effortlessly.' },
        { icon: '🔍', title: 'Keyword Hunter', desc: 'Monitors conversations needing human attention and alerts you instantly.' },
        { icon: '📊', title: 'Smart Weekly Report', desc: 'Comprehensive weekly report on reply performance, customer satisfaction, and growth opportunities.' },
      ],
    },
    howItWorks: {
      title: '3 Steps and You\'re Ready',
      steps: [
        { num: '01', title: 'Sign Up & Connect', desc: 'Connect your channels in under 3 minutes — Instagram, WhatsApp, Facebook, Gmail, and more.' },
        { num: '02', title: 'Define Your Business', desc: 'Tell the AI about your business, your tone, and your frequently asked questions.' },
        { num: '03', title: 'Sleep Peacefully', desc: 'The AI replies to customers, posts your content, and protects your reputation — automatically.' },
      ],
    },
    pricing: {
      title: 'Choose Your Plan',
      monthly: 'Monthly',
      annual: 'Annual',
      annualSave: 'Save 20%',
      mostPopular: 'Most Popular',
      startFree: 'Get Started',
      plans: [
        { name: 'Starter', nameAr: 'Starter', price: 29, desc: 'Perfect for small businesses', features: ['1 channel', '500 replies/month', 'Basic AI replies', 'Monthly report', 'Email support'] },
        { name: 'Business', nameAr: 'Business', price: 59, popular: true, desc: 'Best for most businesses', features: ['5 channels', '2,000 replies/month', 'Before They Complain', 'Daily auto content', 'Weekly report', 'Live support'] },
        { name: 'Growth', nameAr: 'Growth', price: 89, desc: 'For businesses scaling up', features: ['10 channels', '5,000 replies/month', 'All Business features', 'Keyword Hunter', 'Advanced analytics', 'API access'] },
        { name: 'Agency', nameAr: 'Agency', price: 249, desc: 'Manage multiple clients', features: ['Unlimited channels', 'Unlimited replies', 'Agency dashboard', 'White-label', 'Dedicated account manager', 'Guaranteed SLA'] },
      ],
    },
    testimonials: {
      title: 'What Our Customers Say',
      items: [
        { name: 'Ahmed Al-Salem', business: 'Restaurant, Riyadh', rating: 5, quote: 'After Naz, our replies became instant even while sleeping. Customers noticed and our ratings improved significantly.' },
        { name: 'Sara Al-Mutairi', business: 'Beauty Clinic, Jeddah', rating: 5, quote: 'I used to waste hours daily answering the same questions. Now AI handles everything and I focus on my work.' },
        { name: 'Mohammed Al-Zahrani', business: 'Café, Dammam', rating: 5, quote: 'The Before They Complain system saved me multiple times. I now know about an issue before they leave a bad review.' },
      ],
    },
    faq: {
      title: 'Frequently Asked Questions',
      items: [
        { q: 'Do I need technical experience to use Naz?', a: 'Not at all. Naz is designed to be easy for any business owner. Setup takes less than 10 minutes.' },
        { q: 'Will AI reply for me or suggest replies?', a: 'You can choose both modes — fully automatic replies or suggested replies for you to review and approve.' },
        { q: 'Which channels are supported?', a: 'Instagram, Facebook, WhatsApp, Gmail, TikTok, and X (Twitter). Coming soon: Snapchat and Telegram.' },
        { q: 'Is my data secure?', a: 'Yes. We use end-to-end SSL encryption and your data is never shared with any third party.' },
        { q: 'Can I cancel anytime?', a: 'Yes, you can cancel your subscription at any time with no additional fees.' },
        { q: 'Is there a free trial?', a: 'Yes! All plans come with a 14-day full free trial with no credit card required.' },
      ],
    },
    finalCta: {
      title: 'Start Today — First Reply Free',
      btn: 'Start Your Free Trial',
      noCc: 'No credit card required',
    },
    // Auth
    auth: {
      login: 'Login',
      register: 'Sign Up',
      email: 'Email',
      password: 'Password',
      confirmPassword: 'Confirm Password',
      name: 'Name',
      forgotPassword: 'Forgot password?',
      noAccount: "Don't have an account?",
      hasAccount: 'Already have an account?',
      signUp: 'Sign Up',
      signIn: 'Sign In',
      loginSuccess: 'Login successful',
      registerSuccess: 'Account created successfully',
      loginError: 'Login failed',
      registerError: 'Account creation failed',
      invalidEmail: 'Invalid email',
      invalidPassword: 'Password must be at least 8 characters',
      passwordsMismatch: 'Passwords do not match',
      continueWithGoogle: 'Continue with Google',
      continueWithFacebook: 'Continue with Facebook',
      orDivider: 'OR',
      authFailed: 'Authentication failed. Please try again',
      // Auth Panel
      welcomeBack: 'Welcome back',
      yourSystemAwaits: 'Your system awaits',
      loginToAccess: 'Sign in to access your AI dashboard',
      startFree: 'Start for free',
      noCreditCard: 'No credit card required',
      joinToday: 'Join today',
      systemOnline: 'SYSTEM ONLINE',
      stats: {
        businesses: 'Businesses',
        autoReplies: 'Auto replies',
        avgReply: 'Avg reply',
      },
      features: {
        reply8sec: 'Reply in 8 seconds to every message',
        aiUnderstands: 'AI that understands your language',
        instantReports: 'Instant reports for your business performance',
        freeTrial: '14-day full free trial',
        noCreditCard: 'No credit card needed',
        ready5min: 'Ready in under 5 minutes',
      },
      liveActivity: 'LIVE ACTIVITY',
      activity: {
        ahmedWhatsapp: 'Ahmed — WhatsApp',
        replied: 'Replied ✓',
        saraInstagram: 'Sara — Instagram',
        potentialLead: 'Potential lead ⚡',
      },
      allRightsReserved: '© 2025 Naz. All rights reserved.',
      byContinuing: 'By continuing you agree to our',
      termsPrivacy: 'Terms & Privacy',
      // Password strength
      passwordWeak: 'Weak',
      passwordFair: 'Fair',
      passwordStrong: 'Strong',
      // Register page
      startJourney: 'Start your journey',
      createAccountActivate: 'Create your account and activate your AI in minutes',
      createAccountActivateEn: 'Create your account and activate your AI in minutes',
      // Common
      signingIn: 'Signing in...',
      signingInEn: 'Signing in...',
      creatingAccount: 'Creating account...',
      creatingAccountEn: 'Creating account...',
      // Trust badges
      secure: 'Secure',
      secureEn: 'Secure',
      daysFree: '14 days free',
      daysFreeEn: '14 days free',
      instantSetup: 'Instant setup',
      instantSetupEn: 'Instant setup',
    },
    // AI Knowledge
    aiKnowledge: {
      title: 'AI Knowledge & Instructions',
      subtitle: 'Customize how the AI understands your business and responds to customers',
      knowledgeBase: 'Knowledge Base',
      knowledgeBaseDesc: 'Upload PDF or Excel files containing your business information, menus, price lists, FAQs, or any reference documents.',
      uploadFile: 'Upload File',
      uploadFileDesc: 'Upload PDF or Excel files (max 10MB)',
      noFiles: 'No files uploaded yet',
      uploadedAt: 'Uploaded',
      deleteFile: 'Delete',
      deleteConfirm: 'Are you sure you want to delete this file?',
      extractError: 'Failed to extract text from file',
      uploadSuccess: 'File uploaded successfully',
      uploadError: 'Failed to upload file',
      fileDeleted: 'File deleted successfully',
      customInstructions: 'Custom AI Instructions',
      customInstructionsDesc: 'Tell the AI how to behave, what tone to use, and specific rules for your business.',
      instructionsPlaceholder: 'Always greet customers warmly. Never promise discounts without checking with the owner. If someone asks about delivery outside Riyadh, tell them to call us directly.',
      saveInstructions: 'Save Instructions',
      instructionsSaved: 'Instructions saved successfully',
      instructionsError: 'Failed to save instructions',
      testAi: 'Test AI Response',
      testAiDesc: 'See how the AI responds with your current knowledge and instructions.',
      testQuestion: 'Test Question',
      testQuestionPlaceholder: 'Type a customer question to test the AI response...',
      testResponse: 'AI Response',
      testButton: 'Test Response',
      testing: 'Testing...',
      testError: 'Failed to generate test response',
      supportedFormats: 'Supported formats: PDF, Excel (XLSX, XLS)',
      maxSize: 'Maximum file size: 10MB',
    },
    // Dashboard
    dashboard: {
      title: 'Dashboard',
      totalMessages: 'Total Messages',
      aiReplies: 'AI Replies',
      responseRate: 'Response Rate',
      activeChannels: 'Active Channels',
      recentConversations: 'Recent Conversations',
      noChannels: 'No channels connected yet',
      connectChannel: 'Connect Channel',
      vsYesterday: 'vs yesterday',
    },
    // Inbox
    inbox: {
      title: 'Inbox',
      noConversations: 'No conversations',
      noResults: 'No results found',
      noMessages: 'No messages',
      selectConversation: 'Select a conversation to start',
      typeMessage: 'Type your message...',
      send: 'Send',
      aiMode: 'Auto-Reply',
      aiModeActive: 'Auto-Reply Active',
      aiModeInactive: 'Auto-Reply Paused',
      typing: 'Typing...',
      manualReply: 'Manual Reply',
      aiReply: 'AI Reply',
    },
    // Channels
    channels: {
      title: 'Channels',
      subtitle: 'Connect your channels and enable AI auto-reply',
      connect: 'Connect',
      connected: 'Connected',
      notConnected: 'Not connected',
      disconnected: 'Disconnected',
      reconnect: 'Reconnect',
      disconnect: 'Disconnect',
      autoReplyActive: 'Auto-Reply Active',
      autoReplyPaused: 'Auto-Reply Paused',
      connectFacebook: 'Connect Facebook',
      connectInstagram: 'Connect Instagram',
      connectGmail: 'Connect Gmail',
      connectWhatsApp: 'Connect WhatsApp',
      lastMessage: 'Last message',
      noChannels: 'No channels connected',
      willNeedSignIn: 'You will need to sign in to grant access',
      permissionText: 'Naz will request permission to access your {channel} messages and reply automatically with AI.',
      confirmDisconnect: 'Are you sure you want to disconnect this channel?',
      connectedSuccess: 'Channel connected successfully',
      connectionCancelled: 'Connection cancelled',
      connectionFailed: 'Failed to connect channel',
      disconnected2: 'Channel disconnected',
      disconnectFailed: 'Failed to disconnect channel',
      updateFailed: 'Failed to update settings',
      aiOn: 'AI: Enabled',
      aiOff: 'AI: Disabled',
      aiEnabled: 'AI auto-reply enabled',
      aiDisabled: 'AI auto-reply disabled',
      businessPlan: 'Business plan',
      upgradeNow: 'Upgrade Now',
    },
    // Tools
    tools: {
      title: 'Free Tools',
      tryIt: 'Try It',
      freeUses: 'Free uses remaining today',
      limitReached: 'Daily limit reached',
      signUpForUnlimited: 'Sign up for unlimited access',
    },
    // Blog
    blog: {
      title: 'Blog',
      readMore: 'Read More',
      backToBlog: 'Back to Blog',
      relatedTools: 'Related Free Tools',
      upsell: 'Discover how Naz Autoreply can double your sales',
      tryFree: 'Try Free',
    },
    // Common
    common: {
      loading: 'Loading...',
      error: 'An error occurred',
      retry: 'Retry',
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      edit: 'Edit',
      close: 'Close',
      search: 'Search',
      filter: 'Filter',
      all: 'All',
      profile: 'Profile',
      logout: 'Logout',
      continue: 'Continue',
      back: 'Back',
    },
    // Footer
    footer: {
      tagline: 'Your AI employee that never sleeps',
      product: 'PRODUCT',
      productLinks: ['Features', 'Pricing', 'Integrations', 'Security'],
      company: 'COMPANY',
      companyLinks: ['About Naz', 'Blog', 'Careers', 'Contact'],
      support: 'SUPPORT',
      supportLinks: ['Help Center', 'Documentation', 'System Status', 'Privacy'],
      copyright: '© 2026 Naz. All rights reserved',
    },
    // Empty States
    empty: {
      noData: 'No data available',
      noMessages: 'No messages',
      noChannels: 'No channels connected',
      noReports: 'No reports',
      noPosts: 'No posts',
      getStarted: 'Get Started',
    },
  },
}

export type Language = 'ar' | 'en'
export type TranslationKey = keyof typeof translations.ar
