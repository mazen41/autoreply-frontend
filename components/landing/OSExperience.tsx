...
const IC = {
  ...
  DollarSign: ({ c = 'var(--text-secondary)', s = 20 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 100 7h5a3.5 3.5 0 010 7H6"/>
    </svg>
  ),
  Inbox: ({ c = 'var(--text-secondary)', s = 20 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/>
      <path d="M5.45 5.11L2 12v6a2 2 0 002 2h16a2 2 0 002-2v-6l-3.45-6.89A2 2 0 0016.76 4H7.24a2 2 0 00-1.79 1.11z"/>
    </svg>
  ),
  Moon: ({ c = 'var(--text-secondary)', s = 20 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/>
    </svg>
  ),
  TrendingUp: ({ c = 'var(--accent)', s = 20 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/>
    </svg>
  ),
  CheckCircle: ({ c = 'var(--accent)', s = 20 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>
    </svg>
  ),
  Clock: ({ c = 'var(--text-secondary)', s = 20 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
    </svg>
  ),
  Shield: ({ c = 'var(--accent)', s = 14 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
    </svg>
  ),
  Sparkle: ({ c = 'var(--accent)', s = 14 }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div className="relative flex items-center justify-center select-none" style={{ width: s, height: s }}>
      ...
      <div className="relative core-glow flex items-center justify-center" style={{
        ...
        <svg width={s * 0.18} height={s * 0.18} viewBox="0 0 24 24" fill="none">
          <path d="M12 2l2.4 7.4H22l-6.2 4.5 2.4 7.4L12 17l-6.2 4.3 2.4-7.4L2 9.4h7.6L12 2z"
            fill="var(--accent)" style={{ filter: 'drop-shadow(0 0 8px var(--accent-subtle))' }}/>
        </svg>
      </div>
    ...
    <motion.div className="absolute inset-0 flex items-center justify-center z-10 px-6"
      ...
      <div className="w-full max-w-4xl flex flex-col lg:flex-row items-center gap-8">
        ...
        <div className="flex-1 max-w-sm w-full">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)' }}>
            <div className="px-4 py-2.5 flex items-center gap-2" style={{ borderBottom: '1px solid var(--divider)' }}>
              <div className="w-1.5 h-1.5 rounded-full status-live" style={{ background: 'var(--accent)' }} />
              <span className="text-[10px] font-bold" style={{ color: 'var(--text-tertiary)' }}>5 {isRTL ? 'محادثة نشطة' : 'active conversations'}</span>
            </div>
            <div className="p-2 space-y-1.5">
              {MSGS.map((msg, i) => (
                <div key={msg.from} className="flex items-center gap-2.5 p-2.5 rounded-xl transition-all duration-300"
                  style={{ background: activeMsg === i ? 'var(--accent-subtle)' : 'var(--surface-elevated)', borderLeft: activeMsg === i ? '2px solid var(--accent-subtle)' : '2px solid transparent' }}>
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                    style={{ background: `${msg.color}12`, border: `1px solid ${msg.color}20` }}>
                    <msg.Icon c={msg.color} s={14} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[11px] font-bold" style={{ color: 'var(--text-primary)' }}>{msg.from}</div>
                  ...
                  {activeMsg === i ? (
                    <div className="flex gap-0.5 flex-shrink-0">
                      {[0,1,2].map(j => <div key={j} style={{ width: 3, height: 3, borderRadius: '50%', background: 'var(--accent)', animation: `typingDot 1.2s ease ${j*0.2}s infinite` }} />)}
                    </div>
                  ) : (
        ...
        <div className="flex-1 max-w-xs w-full">
          ...
          <div className="rounded-2xl p-4" style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)' }}>
            {[
              { l: isRTL ? 'تحليل النص'   : 'Text Analysis',    p: 94, c: 'var(--accent)' },
              { l: isRTL ? 'توليد الرد'   : 'Response Gen',     p: 88, c: 'var(--accent)' },
              { l: isRTL ? 'إدارة السياق' : 'Context Handling', p: 97, c: 'var(--accent)' },
              { l: isRTL ? 'اكتشاف النية' : 'Intent Detection', p: 91, c: 'var(--accent)' },
            ].map((b, i) => (
            ...
            <div className="pt-3 mt-3 flex items-center justify-between" style={{ borderTop: '1px solid var(--divider)' }}>
              <span className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>{isRTL ? 'متوسط الرد' : 'Avg reply'}</span>
              <span className="text-sm font-black" style={{ color: 'var(--accent)' }}>8s</span>
            </div>
          </div>
        </div>
      </div>
...
function State4({ op, isRTL }: { op: MotionValue<number>; isRTL: boolean }) {
  ...
  const flow = [
    ...
    { Icon: IC.Sparkle,       ar: 'ذكاء',   en: 'AI',      c: 'var(--accent)' },
    { Icon: IC.Zap,           ar: 'إجراء',  en: 'Action',  c: 'var(--accent)' },
    { Icon: IC.Target,        ar: 'نتيجة',  en: 'Result',  c: 'var(--accent)' },
  ]
  const cards = [
          ...
          <React.Fragment key={i}>
            <div className="flex flex-col items-center gap-2 transition-all duration-300"
              ...
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center transition-all duration-300"
                style={{
                  background: step === i ? 'var(--accent-subtle)' : 'var(--surface-elevated)',
                  border: `1.5px solid ${step === i ? 'var(--accent)' : 'var(--border)'}`,
                  boxShadow: step === i ? '0 0 28px var(--accent-subtle)' : 'none',
                }}>
                <s.Icon c={step === i ? 'var(--accent)' : 'var(--text-tertiary)'} s={22} />
              </div>
              <span className="text-xs font-bold" style={{ color: step === i ? 'var(--accent)' : 'var(--text-tertiary)' }}>
                {isRTL ? s.ar : s.en}
              </span>
            </div>
            {i < flow.length - 1 && (
      ...
      <div className="flex flex-wrap justify-center gap-4">
        {cards.map((c, i) => (
          <div key={i} className="rounded-2xl p-5 w-44 sm:w-52"
            ...
            <div className="mb-3"><c.Icon c="var(--accent)" s={22} /></div>
            <div className="text-sm font-bold mb-2" style={{ color: 'var(--text-primary)' }}>{isRTL ? c.ar : c.en}</div>
            <div className="text-[11px] font-bold" style={{ color: 'var(--accent)' }}>{isRTL ? c.r_ar : c.r_en}</div>
          </div>
        ))}
      </div>
    </motion.div>
  ...
  return (
    <motion.div className="absolute inset-0 flex flex-col items-center justify-center z-10 px-6"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-3xl mb-8">
        {OUTCOMES.map((item, i) => (
          <div key={i} className="card-os rounded-2xl p-5 text-center"
            style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)' }}>
            <div className="flex justify-center mb-2"><item.Icon c="var(--accent)" s={20} /></div>
            <div className="text-[10px] mb-2" style={{ color: 'var(--accent)' }}>
              {isRTL ? 'قبل: ' : 'Before: '}{item.before}
            </div>
            <div className="text-3xl font-black" style={{ color: 'var(--accent)', letterSpacing: '-0.04em' }}>
              {vals[i]}
            </div>
            ...
            <div className="flex items-center justify-center gap-1 mt-2">
              <IC.TrendingUp c="var(--accent)" s={10} />
              <span className="text-[10px] font-bold" style={{ color: 'var(--accent)' }}>{isRTL ? 'تحسن' : 'Improved'}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl p-5 w-full max-w-xl"
        style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)' }}>
        <div className="text-[10px] font-bold tracking-widest mb-4 text-center" style={{ color: 'var(--text-tertiary)' }}>
          {isRTL ? 'متوسط النتائج — بعد 30 يوم' : 'AVERAGE RESULTS — AFTER 30 DAYS'}
        </div>
        {[
          { l: isRTL ? 'رضا العملاء'  : 'Customer Satisfaction', p: 94, c: 'var(--accent)' },
          { l: isRTL ? 'سرعة الرد'    : 'Response Speed',        p: 98, c: 'var(--accent)' },
          { l: isRTL ? 'معدل التحويل' : 'Conversion Rate',       p: 41, c: 'var(--accent)' },
        ].map((b, i) => (
          <div key={i} className="mb-3 last:mb-0">
          ...
          const inner = (
            <div className="relative rounded-2xl p-4 flex flex-col h-full"
              ...
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-black" style={{ color: isPop ? 'var(--accent)' : 'var(--text-primary)' }}>{name}</h3>
                <IC.Sparkle c={isPop ? 'var(--accent)' : 'var(--text-tertiary)'} s={13} />
              </div>
              <div className="flex items-end gap-1 mb-3">
                <span className="text-2xl font-black" style={{ color: isPop ? 'var(--accent)' : 'var(--text-primary)', letterSpacing: '-0.04em' }}>
