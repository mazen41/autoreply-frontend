            ...
            return (
              <motion.div key={key} initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1, x: message ? [0, -7, 7, -5, 5, 0] : 0 }} transition={{ delay: 0.12 + i * 0.12, duration: message ? 0.3 : 0.45 }}>
                ...
                <div className="relative">
                  ...
                  <input id={`login-${key}`} type={type} required autoComplete={autoComplete} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })} onBlur={e => { setTouched({ ...touched, [key]: true }); e.currentTarget.style.boxShadow = 'none' }} placeholder={ph} aria-invalid={!!message} aria-describedby={describedBy} className="w-full py-3 rounded-xl text-sm outline-none transition-all duration-200" style={{ background: 'var(--surface-elevated)', border: `1px solid ${message ? 'var(--error)' : 'var(--border)'}`, color: 'var(--text-primary)', paddingInlineStart: 42, paddingInlineEnd: hasTrailingButton ? 44 : 16, boxShadow: message ? '0 0 0 3px color-mix(in srgb, var(--error) 14%, transparent)' : undefined }} onFocus={e => { e.currentTarget.style.borderColor = message ? 'var(--error)' : 'var(--accent-focus)'; e.currentTarget.style.boxShadow = message ? '0 0 0 3px color-mix(in srgb, var(--error) 14%, transparent)' : '0 0 0 3px var(--accent-subtle)' }} />
